...
export default function ProductDetail() {
  ...
  return (
    <ScrollView className="flex-1 bg-white pt-20 px-6">
      ...
      <Text className="text-2xl font-VictorMonoBold mb-8">Détail du produit</Text>
      ...
      <Text className="font-SpaceGroteskBold text-gray-700 mb-1">Code postal</Text>
      <TextInput
        className="border border-gray-300 rounded-lg p-3 mb-4 font-SpaceGroteskRegular"
        value={zipcode}
      ...
      <Text className="font-SpaceGroteskBold text-gray-700 mb-1">Catégorie</Text>
      ...
      <Text className="font-SpaceGroteskBold text-gray-700 mb-1">Clé de sécurité</Text>
      ...
      {history.length === 0 ? (
        <Text className="text-gray-400 font-SpaceGroteskRegular mb-10">Aucun historique.</Text>
      ) : (
        [...history].reverse().map((entry, index) => {
          const { label, color } = stateLabel(entry.code);
          return (
            <View key={index} className="flex-row justify-between items-center border-b border-gray-100 py-3">
              <Text className={`font-SpaceGroteskBold ${color}`}>{label}</Text>
              <Text className="text-gray-400 font-SpaceGroteskRegular text-sm">{formatDate(entry.date)}</Text>
            </View>
          );
        })
      )}
